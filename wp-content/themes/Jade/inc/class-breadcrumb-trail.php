...
function breadcrumb_trail_items($args = array()){
	if ( function_exists( 'is_bbpress' ) && is_bbpress() )
		$breadcrumb = new bbPress_Breadcrumb_Trail( $args );
	else
		$breadcrumb = new Breadcrumb_Trail( $args );
...
class Breadcrumb_Trail {
	...
	public function trail() {
		...
		if ( !empty( $this->items ) && is_array( $this->items ) ) {
			...
			$breadcrumb = "\n\t\t" . '<' . tag_escape( $this->args['container'] ) . ' class="breadcrumb-trail breadcrumbs met_breadcrumb met_clean_list hidden-768" itemprop="breadcrumb">';
			...
			$separator = ( !empty( $this->args['separator'] ) ? '<li class="sep met_color2">' . $this->args['separator'] . '</li>' : '<li class="sep">/</li>' );

			/* Add li tags - @muratkaracam */
			if($this->items){
				foreach($this->items as $br_item){
					$br_item = str_replace('<a','<a class="met_color2 met_color_transition"',$br_item); //add primary color class
					$new_items[] = '<li>'.$br_item.'</li>';
				}
			}
	...
	public function do_post_hierarchy( $post_id ) {
		...
		elseif ( false !== $post_type_object->rewrite ) {
			...
			if ( isset($post_type_object->rewrite['with_front']) AND $post_type_object->rewrite['with_front'] )
				$this->do_rewrite_front_items();
