...
add_action('dslc_hook_register_modules',
    create_function('', 'return dslc_register_module( "MET_ContentGrid5" );')
);

class MET_ContentGrid5 extends DSLC_Module {

    var $module_id = 'MET_ContentGrid5';
    var $module_title = 'Modern';
    ...
    function options() {
        ...
        $dslc_options = array_merge(
            ...
            // Title
            lc_general('.met_portfolio_item_title h3', 'Title', array('color' => '', 'color:hover' => '','font-size' => '24','line-height' => '27','font-weight' => '600')),
    ...
    function output( $options ) {
        ...
        if ( $dslc_query->have_posts() ) {
            ...
            while ( $dslc_query->have_posts() ) : $dslc_query->the_post();
                ...
                <div class="col-md-<?php echo $columns; ?> met_portfolio_item met_portfolio_item_6 met_isotope_item <?php if(!empty($taxonomies)){foreach($taxonomies as $taxonomy) echo $taxonomy['slug'].' ';} ?> <?php echo $met_shared_options['classes'].' '.$met_shared_options['uniqueClass']; ?>" <?php echo $met_shared_options['data-']; ?>>
                    <div class="met_portfolio_item_6_inner_wrap">
                        <?php
                        ...
                        <div class="met_portfolio_item_details_6 clearfix">
                            ...
                            <?php if ( $elements == 'all' || in_array( 'title', $elements ) || in_array( 'categories', $elements ) || $linkBox != '' || $lbBox != '' ) : ?>
                                <div class="met_portfolio_item_details_6_title_links">
                                    <?php if ( $elements == 'all' || in_array( 'title', $elements ) ) : ?>
                                        <a href="<?php the_permalink(); ?>" class="met_portfolio_item_title"><h3><?php the_title(); ?></h3></a>
                                    <?php endif; ?>
                                    <?php if( $linkBox != '' || $lbBox != '' ){ ?><div class="met_portfolio_item_links"><?php echo $linkBox.$lbBox; ?></div><!-- .met_portfolio_item_links --><?php } ?>
                                </div>
                                <?php if( ($elements == 'all' || in_array( 'categories', $elements )) && !empty($taxonomies) ) : ?>
                                    <div class="met_portfolio_item_details_6_cats">
                                        <?php
                                        $categories = array();
                                        foreach($taxonomies as $category)
                                            $categories[] = '<a href="'.(array_key_exists('link',$category) ? $category['link'] : get_term_link( $category["slug"], $category["taxonomy"] )).'">'.$category["name"].'</a>';
                    ...
                    </div><!-- .met_portfolio_item_6_inner_wrap -->
